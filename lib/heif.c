...
/* decode_thumbnail: returns 0=failure, 1=success, 2=no thumbnail */
...
int decode_thumbnail(uint8_t *heic_in, int heic_in_size, int config_only, uint32_t *width, uint32_t *height,
    ...
    int n_thumbs = heif_image_handle_get_number_of_thumbnails(handle);
    if(n_thumbs == 0) {
        heif_image_handle_release(handle);
        heif_context_free(context);
        return 2;
    ...
    err = heif_image_handle_get_thumbnail(handle, thumb_id, &thumb_handle);
    if(err.code != heif_error_Ok) {
        heif_image_handle_release(handle);
        heif_context_free(context);
        return 0;
    ...
    *width = (uint32_t)heif_image_handle_get_width(thumb_handle);
    *height = (uint32_t)heif_image_handle_get_height(thumb_handle);

    *is_premultiplied = (uint32_t)heif_image_handle_is_premultiplied_alpha(thumb_handle);

    err = heif_image_handle_get_preferred_decoding_colorspace(thumb_handle, colorspace, chroma);
    if(err.code != heif_error_Ok) {
    ...
    err = heif_decode_image(thumb_handle, &img, *colorspace, *chroma, options);
    if(err.code != heif_error_Ok) {
        heif_image_handle_release(thumb_handle);
        heif_image_handle_release(handle);
        heif_context_free(context);
